...
object AaBtnEventHook: AaHook() {
    ...
    override fun hook(config: SharedPreferences, ctx: XposedRuntimeContext) {
        ...
        ctx.findAllMethods(ContextWrapper::class.java) {
            name == "registerReceiver"
        }.forEach { registerReceiver ->
            ctx.hookBefore(registerReceiver) { param ->
                runCatching {
...
//                log(tagName,"registerReceiver->$clazzName:${mActions.joinToString()}")
                    try {
                        ctx.hookBefore(ctx.findMethod(clazz, findSuper = true) {
                            name == "onReceive"
                            && parameterCount == 2
                    ...
                    log(tagName, "btnEvent registerReceiver", e)
                }
            }
        }
    }
